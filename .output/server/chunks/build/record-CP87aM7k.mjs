...
import { defineComponent, withAsyncContext, computed, watch, reactive, ref, nextTick, mergeProps, withCtx, createTextVNode, createVNode, unref, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderStyle, ssrInterpolate, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderAttr, ssrRenderClass } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const form = reactive({
      supplierId: qSupplierId != null ? qSupplierId : "",
      poId: qPoId != null ? qPoId : "",
    ...
    function applyCredit() {
      ...
      if (!credit) return;
      if (po) {
        const due = Number((_a = po.balance_payable) != null ? _a : 0);
        form.amount = Math.max(0, due - credit) || null;
        success(`Credit of \u09F3${credit.toLocaleString()} applied \u2014 new amount: \u09F3${((_b = form.amount) != null ? _b : 0).toLocaleString()}`);
      } else {
        form.amount = null;
    ...
    if (qUseCredit) {
      nextTick(() => {
        if (availableCredit.value > 0) applyCredit();
      });
    }
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="grid grid-cols-1 lg:grid-cols-3 gap-6"><div class="lg:col-span-2 glass-card p-6 space-y-5"><h3 class="section-title">Payment Details</h3><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Supplier *</label>`);
      ...
      if (unref(availableCredit) > 0) {
        _push(`<div class="rounded-xl p-3.5 border" style="${ssrRenderStyle({ "background": "rgba(59,130,246,0.06)", "border-color": "rgba(59,130,246,0.20)" })}"><div class="flex items-center justify-between"><div><p class="text-xs font-semibold text-blue-300">Available Supplier Credit</p><p class="text-lg font-bold font-mono text-blue-400 mt-0.5">\u09F3${ssrInterpolate(unref(availableCredit).toLocaleString())}</p></div><button type="button" class="text-xs px-3 py-1.5 rounded-lg bg-blue-500/20 text-blue-300 border border-blue-500/30 hover:bg-blue-500/30 transition-colors"> Apply Credit </button></div><p class="text-[11px] text-blue-400/50 mt-1">Credit will reduce the payment amount required from this supplier</p></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Purchase Order *</label><select class="input-glass"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).poId) ? ssrLooseContain(unref(form).poId, "") : ssrLooseEqual(unref(form).poId, "")) ? " selected" : ""}>\u2014 Select PO \u2014</option><!--[-->`);
...
//# sourceMappingURL=record-CP87aM7k.mjs.map
