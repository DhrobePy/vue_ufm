  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        if (Number(unref(grn).expected_quantity) > 0) {
          _push(`<tr><td class="px-4 py-3 text-gray-400">Expected Quantity</td><td class="px-4 py-3 text-right font-mono text-purple-400">${ssrInterpolate(Number(unref(grn).expected_quantity).toLocaleString())}</td><td class="px-4 py-3 text-right font-mono text-purple-400">\u09F3${ssrInterpolate(unref(expectedValue).toLocaleString(void 0, { maximumFractionDigits: 0 }))}</td></tr>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<tr><td class="px-4 py-3 text-gray-200 font-semibold">Actual Quantity Received</td><td class="px-4 py-3 text-right font-mono text-emerald-400 font-semibold">${ssrInterpolate(Number(unref(grn).quantity_received_kg).toLocaleString())}</td><td class="px-4 py-3 text-right font-mono text-emerald-400 font-semibold">\u09F3${ssrInterpolate(Number(unref(grn).total_value).toLocaleString(void 0, { maximumFractionDigits: 0 }))}</td></tr>`);
        if (Number(unref(grn).expected_quantity) > 0) {
          _push(`<tr><td class="${ssrRenderClass([unref(varianceColor), "px-4 py-3 font-semibold"])}">Variance (Quantity)</td><td class="${ssrRenderClass([unref(varianceColor), "px-4 py-3 text-right font-mono font-semibold"])}">${ssrInterpolate(unref(varianceKg) > 0 ? "+" : "")}${ssrInterpolate(unref(varianceKg).toLocaleString(void 0, { maximumFractionDigits: 2 }))} <span class="text-[10px] ml-1">(${ssrInterpolate(unref(varianceKg) > 0 ? "+" : "")}${ssrInterpolate(unref(variancePct).toFixed(2))}%)</span></td><td class="${ssrRenderClass([unref(varianceColor), "px-4 py-3 text-right font-mono font-semibold"])}">${ssrInterpolate(unref(varianceKg) > 0 ? "+" : "")}\u09F3${ssrInterpolate((unref(varianceKg) * Number(unref(grn).unit_price_per_kg || unref(grn).po_unit_price || 0)).toLocaleString(void 0, { maximumFractionDigits: 0 }))}</td></tr>`);
        } else {
          _push(`<!---->`);
        }
        if (Number(unref(grn).expected_quantity) > 0) {
          _push(`<tr class="bg-purple-500/[0.06] border-t-2 border-purple-500/20"><td class="px-4 py-3 font-bold text-gray-200 uppercase text-xs tracking-wider">EXPECTED PAYABLE</td><td class="px-4 py-3 text-right font-mono font-semibold text-gray-300">${ssrInterpolate(Number(unref(grn).expected_quantity).toLocaleString())} KG</td><td class="px-4 py-3 text-right font-mono font-bold text-purple-400">\u09F3${ssrInterpolate(unref(expectedValue).toLocaleString(void 0, { maximumFractionDigits: 0 }))}</td></tr>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</tbody></table></div>`);
        if (unref(grn).variance_remarks) {
        ...
        _push(`</div></div><div class="space-y-5"><div class="glass-card p-5 border border-gold-500/20 bg-gold-500/[0.03]"><p class="text-xs text-gray-500 mb-1">EXPECTED PAYABLE (Amount Due)</p><p class="text-2xl font-bold text-gold-400 font-mono"> \u09F3${ssrInterpolate((Number(unref(grn).expected_quantity) > 0 ? unref(expectedValue) : Number(unref(grn).total_value)).toLocaleString(void 0, { maximumFractionDigits: 0 }))}</p>`);
        if (Number(unref(grn).expected_quantity) > 0) {
          _push(`<p class="text-[10px] text-gray-600 mt-1"> Expected Qty \xD7 Unit Price </p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
        if (Number(unref(grn).expected_quantity) > 0) {
          _push(`<div class="glass-card p-4 border border-white/[0.06]"><p class="text-xs text-gray-500 mb-0.5">Actual Received Value (for reference)</p><p class="text-lg font-semibold text-gray-400 font-mono">\u09F3${ssrInterpolate(Number(unref(grn).total_value).toLocaleString(void 0, { maximumFractionDigits: 0 }))}</p></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(grn).grn_status !== "cancelled") {
...
//# sourceMappingURL=index-CeWLtNl0.mjs.map
