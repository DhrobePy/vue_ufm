...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const isValid = computed(
      () => form.supplierId && form.poId && form.date && form.method && form.bankAccountId && form.amount && form.amount > 0 && (form.method === "cash" || form.reference)
    );
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></select></div><div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Payment Date *</label><input${ssrRenderAttr("value", unref(form).date)} type="date" class="input-glass"></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Payment Method *</label><select class="input-glass"><option value="cash"${ssrIncludeBooleanAttr(Array.isArray(unref(form).method) ? ssrLooseContain(unref(form).method, "cash") : ssrLooseEqual(unref(form).method, "cash")) ? " selected" : ""}>Cash</option><option value="bank"${ssrIncludeBooleanAttr(Array.isArray(unref(form).method) ? ssrLooseContain(unref(form).method, "bank") : ssrLooseEqual(unref(form).method, "bank")) ? " selected" : ""}>Bank Transfer / BEFTN</option><option value="cheque"${ssrIncludeBooleanAttr(Array.isArray(unref(form).method) ? ssrLooseContain(unref(form).method, "cheque") : ssrLooseEqual(unref(form).method, "cheque")) ? " selected" : ""}>Cheque</option></select></div></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Amount (\u09F3) *</label><div class="relative"><span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-bold">\u09F3</span><input${ssrRenderAttr("value", unref(form).amount)} type="number" min="1" class="input-glass pl-8 font-mono text-lg font-bold" placeholder="0"></div>`);
      if (unref(selectedPO)) {
      ...
      if (unref(form).method !== "cash") {
        _push(`<div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Reference / Cheque No. *</label><input${ssrRenderAttr("value", unref(form).reference)} type="text" class="input-glass font-mono" placeholder="BEFTN ref, cheque no., or TxID"></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Company Bank Account *</label><select class="input-glass"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).bankAccountId) ? ssrLooseContain(unref(form).bankAccountId, "") : ssrLooseEqual(unref(form).bankAccountId, "")) ? " selected" : ""}>\u2014 Paid from \u2014</option><!--[-->`);
      ssrRenderList(unref(bankAccounts), (ba) => {
        _push(`<option${ssrRenderAttr("value", ba.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).bankAccountId) ? ssrLooseContain(unref(form).bankAccountId, ba.id) : ssrLooseEqual(unref(form).bankAccountId, ba.id)) ? " selected" : ""}>${ssrInterpolate(ba.bank_name)} \u2014 ${ssrInterpolate(ba.account_number)}</option>`);
      });
...
//# sourceMappingURL=record-CPZYRh_h.mjs.map
