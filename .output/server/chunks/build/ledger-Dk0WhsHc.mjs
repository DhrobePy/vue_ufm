...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    function exportCsv() {
      ...
      const rows = ledger.value.map((tx) => [tx.date, tx.description, tx.ref, tx.debit, tx.credit, tx.balance]);
      const csv = [headers, ...rows].map((row) => row.map((v) => `"${String(v != null ? v : "").replace(/"/g, '""')}"`).join(",")).join("\n");
      const blob = new Blob([csv], { type: "text/csv" });
      const url = URL.createObjectURL(blob);
      const name = `ledger-${(_b = (_a = sup == null ? void 0 : sup.company_name) == null ? void 0 : _a.replace(/\s+/g, "-")) != null ? _b : supplierId}-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv`;
      const a = Object.assign((void 0).createElement("a"), { href: url, download: name });
      a.click();
...
//# sourceMappingURL=ledger-Dk0WhsHc.mjs.map
