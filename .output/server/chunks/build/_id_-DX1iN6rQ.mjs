    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(driver)) {
        ...
        if (unref(activeTab) === "documents") {
          _push(`<div><div class="glass-card p-5"><h3 class="section-title mb-4">Driver Documents</h3>`);
          if (!unref(documents).length) {
            _push(`<div class="text-center py-6 text-gray-600 text-sm">No documents recorded</div>`);
          } else {
            _push(`<table class="w-full text-xs"><thead><tr class="border-b border-white/[0.06]"><th class="pb-2 text-left text-gray-500">Type</th><th class="pb-2 text-left text-gray-500">Document No</th><th class="pb-2 text-left text-gray-500">Issue Date</th><th class="pb-2 text-left text-gray-500">Expiry Date</th><th class="pb-2 text-left text-gray-500">Authority</th><th class="pb-2 text-left text-gray-500">Status</th></tr></thead><tbody><!--[-->`);
            ssrRenderList(unref(documents), (d) => {
              _push(`<tr class="border-b border-white/[0.03]"><td class="py-2 text-gray-300 font-medium">${ssrInterpolate(d.document_type)}</td><td class="py-2 font-mono text-gray-400">${ssrInterpolate(d.document_no || "\u2014")}</td><td class="py-2 text-gray-400">${ssrInterpolate(d.issue_date || "\u2014")}</td><td class="${ssrRenderClass([isExpired(d.expiry_date) ? "text-red-400" : isExpiringSoon(d.expiry_date) ? "text-amber-400" : "text-gray-400", "py-2"])}">${ssrInterpolate(d.expiry_date || "\u2014")}</td><td class="py-2 text-gray-400">${ssrInterpolate(d.issuing_authority || "\u2014")}</td><td class="py-2"><span class="${ssrRenderClass([isExpired(d.expiry_date) ? "bg-red-500/10 text-red-400" : isExpiringSoon(d.expiry_date) ? "bg-amber-500/10 text-amber-400" : "bg-emerald-500/10 text-emerald-400", "badge text-[10px]"])}">${ssrInterpolate(isExpired(d.expiry_date) ? "Expired" : isExpiringSoon(d.expiry_date) ? "Expiring Soon" : "Valid")}</span></td></tr>`);
            });
            _push(`<!--]--></tbody></table>`);
          }
          _push(`</div></div>`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/fleet/drivers/[id].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_id_-DX1iN6rQ.mjs.map
