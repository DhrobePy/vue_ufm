    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(currentStep) === 0) {
        ...
        if (unref(customerDropdownOpen) && unref(filteredCustomers).length) {
          _push(`<div class="absolute left-0 right-0 top-full mt-1 z-30 glass-card shadow-2xl rounded-xl max-h-56 overflow-y-auto py-1"><!--[-->`);
          ssrRenderList(unref(filteredCustomers), (c) => {
            _push(`<button type="button" class="w-full text-left px-4 py-2.5 hover:bg-white/[0.06] transition-colors"><span class="text-sm text-gray-200 font-medium">${ssrInterpolate(c.name)}</span>`);
            if (c.business) {
          ...
          _push(`<div class="absolute left-0 right-0 top-full mt-1 z-30 glass-card rounded-xl py-3 text-center text-xs text-gray-600"> No customers match &quot;${ssrInterpolate(unref(customerQuery))}&quot; </div>`);
        } else {
          _push(`<!---->`);
        }
...
//# sourceMappingURL=create-BsopQbfq.mjs.map
