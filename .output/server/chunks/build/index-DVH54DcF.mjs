  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        _push(`<button${ssrIncludeBooleanAttr(unref(deleting)) ? " disabled" : ""} class="${ssrRenderClass([unref(deleting) ? "opacity-50 cursor-not-allowed" : "", "btn-ghost text-xs w-full justify-start gap-2 text-red-400 hover:text-red-300 hover:border-red-500/30"])}">${ssrInterpolate(unref(deleting) ? "Deleting\u2026" : "\u2715 Delete Payment")}</button></div></div></div><!--]-->`);
      }
      _push(`</div>`);
    };
  }
...
//# sourceMappingURL=index-DVH54DcF.mjs.map
