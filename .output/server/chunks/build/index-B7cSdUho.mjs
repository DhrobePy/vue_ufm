    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(vehicle)) {
        ...
        if (unref(activeTab) === "documents") {
          _push(`<div><div class="glass-card p-5"><div class="flex items-center justify-between mb-4"><h3 class="section-title">Vehicle Documents</h3><button class="btn-gold text-xs">+ Add Document</button></div>`);
          if (!unref(documents).length) {
            _push(`<div class="text-center py-6 text-gray-600 text-sm">No documents recorded</div>`);
          } else {
            _push(`<div class="overflow-x-auto"><table class="w-full text-xs"><thead><tr class="border-b border-white/[0.06]"><th class="pb-2 text-left text-gray-500 font-medium">Document Type</th><th class="pb-2 text-left text-gray-500 font-medium">Document No</th><th class="pb-2 text-left text-gray-500 font-medium">Issue Date</th><th class="pb-2 text-left text-gray-500 font-medium">Expiry Date</th><th class="pb-2 text-left text-gray-500 font-medium">Notes</th><th class="pb-2 text-left text-gray-500 font-medium">Status</th><th class="pb-2"></th></tr></thead><tbody><!--[-->`);
            ssrRenderList(unref(documents), (d) => {
              _push(`<tr class="border-b border-white/[0.03]"><td class="py-2 text-gray-300 font-medium">${ssrInterpolate(d.document_type)}</td><td class="py-2 font-mono text-gray-400">${ssrInterpolate(d.document_number || "\u2014")}</td><td class="py-2 text-gray-400">${ssrInterpolate(d.issue_date || "\u2014")}</td><td class="${ssrRenderClass([isExpiringSoon(d.expiry_date) ? "text-amber-400 font-medium" : isExpired(d.expiry_date) ? "text-red-400 font-medium" : "text-gray-400", "py-2"])}">${ssrInterpolate(d.expiry_date || "\u2014")}</td><td class="py-2 text-gray-400">${ssrInterpolate(d.notes || "\u2014")}</td><td class="py-2"><span class="${ssrRenderClass([isExpired(d.expiry_date) ? "bg-red-500/10 text-red-400" : isExpiringSoon(d.expiry_date) ? "bg-amber-500/10 text-amber-400" : "bg-emerald-500/10 text-emerald-400", "badge"])}">${ssrInterpolate(isExpired(d.expiry_date) ? "Expired" : isExpiringSoon(d.expiry_date) ? "Expiring Soon" : "Valid")}</span></td><td class="py-2 text-right"><button class="text-gray-600 hover:text-red-400 transition-colors">\u2715</button></td></tr>`);
            });
...
//# sourceMappingURL=index-B7cSdUho.mjs.map
