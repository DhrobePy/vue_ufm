...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const isValid = computed(
      () => form.accountId && form.date && form.amount && Number(form.amount) > 0 && form.description.trim()
    );
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_UiPageHeader, {
        ...
        breadcrumb: ["Bank", "New Transaction"]
      }, {
      ...
      _push(`<div class="glass-card p-6 space-y-5"><div class="flex gap-3"><!--[-->`);
      ...
      _push(`<!--]--></div><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Bank Account *</label><select class="field-input"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).accountId) ? ssrLooseContain(unref(form).accountId, "") : ssrLooseEqual(unref(form).accountId, "")) ? " selected" : ""}>Select account\u2026</option><!--[-->`);
      ...
      _push(`<!--]--></select></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Transaction Date *</label><input${ssrRenderAttr("value", unref(form).date)} type="date" class="field-input"></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Amount (\u09F3) *</label><input${ssrRenderAttr("value", unref(form).amount)} type="number" min="1" class="field-input text-lg font-bold" placeholder="0.00"></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Category</label><select class="field-input"><option value="receipt"${ssrIncludeBooleanAttr(Array.isArray(unref(form).category) ? ssrLooseContain(unref(form).category, "receipt") : ssrLooseEqual(unref(form).category, "receipt")) ? " selected" : ""}>Customer Receipt</option><option value="payment"${ssrIncludeBooleanAttr(Array.isArray(unref(form).category) ? ssrLooseContain(unref(form).category, "payment") : ssrLooseEqual(unref(form).category, "payment")) ? " selected" : ""}>Supplier Payment</option><option value="salary"${ssrIncludeBooleanAttr(Array.isArray(unref(form).category) ? ssrLooseContain(unref(form).category, "salary") : ssrLooseEqual(unref(form).category, "salary")) ? " selected" : ""}>Salary / Wages</option><option value="utility"${ssrIncludeBooleanAttr(Array.isArray(unref(form).category) ? ssrLooseContain(unref(form).category, "utility") : ssrLooseEqual(unref(form).category, "utility")) ? " selected" : ""}>Utility / Bill</option><option value="transfer"${ssrIncludeBooleanAttr(Array.isArray(unref(form).category) ? ssrLooseContain(unref(form).category, "transfer") : ssrLooseEqual(unref(form).category, "transfer")) ? " selected" : ""}>Internal Transfer</option><option value="other"${ssrIncludeBooleanAttr(Array.isArray(unref(form).category) ? ssrLooseContain(unref(form).category, "other") : ssrLooseEqual(unref(form).category, "other")) ? " selected" : ""}>Other</option></select></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Reference No.</label><input${ssrRenderAttr("value", unref(form).reference)} class="field-input font-mono" placeholder="Cheque / BEFTN / bKash ref."></div><div class="md:col-span-2 space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Description *</label><textarea rows="2" class="field-input resize-none" placeholder="e.g. Customer payment \u2014 Rahim Traders">${ssrInterpolate(unref(form).description)}</textarea></div><div class="md:col-span-2 space-y-1.5"><label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Notes</label><textarea rows="2" class="field-input resize-none" placeholder="Additional notes\u2026">${ssrInterpolate(unref(form).notes)}</textarea></div></div><div class="rounded-xl p-3.5" style="${ssrRenderStyle({ "background": "rgba(245,158,11,0.05)", "border": "1px solid rgba(245,158,11,0.12)" })}"><p class="text-xs text-gray-500">\u26A0 This transaction will be submitted for maker-checker approval before posting to the account balance.</p></div><div class="flex justify-end gap-3">`);
      ...
      _push(` ${ssrInterpolate(unref(saving) ? "Submitting\u2026" : "Submit Transaction")}</button></div></div></div>`);
    };
  }
});
...
//# sourceMappingURL=create-BUllJM1u.mjs.map
