          ...
          actions: withCtx(({ row }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              if (unref(perms).canDo("credit_sales", "dispatch", "mark_dispatched")) {
                _push2(`<button class="${ssrRenderClass([row.dispatch_hold && !(row.gate_met && row.gate_auto) ? "opacity-40" : "", "btn-gold text-xs py-1 px-2.5"])}"${ssrIncludeBooleanAttr(unref(acting) === row.id) ? " disabled" : ""}${_scopeId}>${ssrInterpolate(unref(acting) === row.id ? "\u2026" : "Dispatch")}</button>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div>`);
              ...
              return [
                createVNode("div", { class: "flex items-center gap-2 justify-end" }, [
                  ...
                  }, toDisplayString(unref(acting) === row.id ? "\u2026" : "Dispatch"), 11, ["onClick", "disabled"])) : createCommentVNode("", true)
                ])
              ];
            }
          }),
...
//# sourceMappingURL=dispatch-bC1QSQwc.mjs.map
