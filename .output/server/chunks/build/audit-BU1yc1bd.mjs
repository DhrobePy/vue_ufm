...
import { defineComponent, ref, withAsyncContext, computed, mergeProps, withCtx, createVNode, unref, resolveDynamicComponent, toDisplayString, openBlock, createBlock, createCommentVNode, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrRenderClass, ssrInterpolate, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderAttr, ssrRenderVNode } from 'vue/server-renderer';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        ssrRenderList(unref(logs), (log) => {
          ssrRenderVNode(_push, createVNode(resolveDynamicComponent(routeFor(log) ? "NuxtLink" : "div"), {
            key: log.id,
            to: routeFor(log) || void 0,
            class: ["flex gap-4 p-4 transition-colors group", routeFor(log) ? "hover:bg-white/[0.04] cursor-pointer" : "hover:bg-white/[0.02]"]
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<div class="flex flex-col items-center shrink-0 mt-1.5"${_scopeId}><div class="${ssrRenderClass(["w-2.5 h-2.5 rounded-full shrink-0", severityDot(log.severity)])}"${_scopeId}></div></div><div class="flex-1 min-w-0"${_scopeId}><div class="flex items-start justify-between gap-3"${_scopeId}><p class="text-sm text-gray-200 leading-snug"${_scopeId}>${ssrInterpolate(log.description)}</p><div class="flex items-center gap-2 shrink-0"${_scopeId}><span class="text-[10px] text-gray-600 font-mono whitespace-nowrap"${_scopeId}>${ssrInterpolate(String(log.created_at).slice(0, 16).replace("T", " "))}</span>`);
                if (routeFor(log)) {
                  _push2(`<span class="text-[10px] text-gold-400/60 group-hover:text-gold-400 transition-colors whitespace-nowrap font-medium"${_scopeId}> View \u2192 </span>`);
                } else if (log.action === "deleted") {
                  _push2(`<span class="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 rounded px-1.5 py-0.5 whitespace-nowrap"${_scopeId}> \u{1F5D1} deleted </span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></div><div class="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1.5"${_scopeId}><span class="text-[11px] text-gray-500 font-mono"${_scopeId}>${ssrInterpolate(log.user_name || "\u2014")}</span>`);
                if (log.module) {
                  _push2(`<span class="${ssrRenderClass(["text-[10px] px-1.5 py-0.5 rounded border font-medium", moduleChip(log.module)])}"${_scopeId}>${ssrInterpolate(moduleLabel(log.module))}</span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`<span class="${ssrRenderClass(["text-[10px] px-1.5 py-0.5 rounded border font-medium", actionChip(log.action)])}"${_scopeId}>${ssrInterpolate(log.action)}</span>`);
                if (log.reference_number) {
                  _push2(`<span class="text-[11px] text-gold-400/80 font-mono"${_scopeId}>${ssrInterpolate(log.reference_number)}</span>`);
                } else {
                  _push2(`<!---->`);
                }
                if (log.severity !== "info") {
                  ...
                  ])}"${_scopeId}>${ssrInterpolate(log.severity)}</span>`);
                } else {
                  _push2(`<!---->`);
                }
                if (log.ip_address) {
                  _push2(`<span class="text-[11px] text-gray-700 font-mono"${_scopeId}>${ssrInterpolate(log.ip_address)}</span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></div>`);
                ...
                return [
                  createVNode("div", { class: "flex flex-col items-center shrink-0 mt-1.5" }, [
                    createVNode("div", {
                      class: ["w-2.5 h-2.5 rounded-full shrink-0", severityDot(log.severity)]
                    }, null, 2)
                  ]),
                  createVNode("div", { class: "flex-1 min-w-0" }, [
                    createVNode("div", { class: "flex items-start justify-between gap-3" }, [
                      createVNode("p", { class: "text-sm text-gray-200 leading-snug" }, toDisplayString(log.description), 1),
                      createVNode("div", { class: "flex items-center gap-2 shrink-0" }, [
                        createVNode("span", { class: "text-[10px] text-gray-600 font-mono whitespace-nowrap" }, toDisplayString(String(log.created_at).slice(0, 16).replace("T", " ")), 1),
                        routeFor(log) ? (openBlock(), createBlock("span", {
                          key: 0,
                          class: "text-[10px] text-gold-400/60 group-hover:text-gold-400 transition-colors whitespace-nowrap font-medium"
                        }, " View \u2192 ")) : log.action === "deleted" ? (openBlock(), createBlock("span", {
                          key: 1,
                          class: "text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 rounded px-1.5 py-0.5 whitespace-nowrap"
                        }, " \u{1F5D1} deleted ")) : createCommentVNode("", true)
                      ])
                    ]),
                    createVNode("div", { class: "flex items-center flex-wrap gap-x-3 gap-y-1 mt-1.5" }, [
                      createVNode("span", { class: "text-[11px] text-gray-500 font-mono" }, toDisplayString(log.user_name || "\u2014"), 1),
                      log.module ? (openBlock(), createBlock("span", {
                        key: 0,
                        class: ["text-[10px] px-1.5 py-0.5 rounded border font-medium", moduleChip(log.module)]
                      }, toDisplayString(moduleLabel(log.module)), 3)) : createCommentVNode("", true),
                      createVNode("span", {
                        class: ["text-[10px] px-1.5 py-0.5 rounded border font-medium", actionChip(log.action)]
                      }, toDisplayString(log.action), 3),
                      log.reference_number ? (openBlock(), createBlock("span", {
                        key: 1,
                        class: "text-[11px] text-gold-400/80 font-mono"
                      }, toDisplayString(log.reference_number), 1)) : createCommentVNode("", true),
                      log.severity !== "info" ? (openBlock(), createBlock("span", {
                        key: 2,
                        class: [
                          "text-[10px] font-medium px-1.5 py-0.5 rounded",
                          log.severity === "critical" ? "bg-red-500/10 text-red-400" : "bg-yellow-500/10 text-yellow-400"
                        ]
                      }, toDisplayString(log.severity), 3)) : createCommentVNode("", true),
                      log.ip_address ? (openBlock(), createBlock("span", {
                        key: 3,
                        class: "text-[11px] text-gray-700 font-mono"
                      }, toDisplayString(log.ip_address), 1)) : createCommentVNode("", true)
                    ])
                  ])
                ];
              }
...
//# sourceMappingURL=audit-BU1yc1bd.mjs.map
