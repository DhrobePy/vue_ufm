...
import { defineComponent, computed, ref, withAsyncContext, mergeProps, withCtx, unref, createTextVNode, openBlock, createBlock, createCommentVNode, createVNode, withModifiers, toDisplayString, useSSRContext } from 'vue';
  ...
  async setup(__props) {
    ...
    async function confirmDelete(row) {
      ...
      try {
        ...
        toastError((_b = (_a = e == null ? void 0 : e.data) == null ? void 0 : _a.statusMessage) != null ? _b : "Failed to delete customer");
      } finally {
        ...
        _push(ssrRenderComponent(_component_UiDataTable, {
          ...
          actions: withCtx(({ row }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              if (unref(isAdminUser) && unref(perms).canDo("customers", "list", "delete")) {
                _push2(`<button${ssrIncludeBooleanAttr(unref(deletingId) === row.id) ? " disabled" : ""} class="btn-ghost text-xs py-1 px-2.5 text-red-400 hover:bg-red-500/10 disabled:opacity-40"${_scopeId}>${ssrInterpolate(unref(deletingId) === row.id ? "\u2026" : "Delete")}</button>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div>`);
              ...
              return [
                createVNode("div", { class: "flex gap-1.5" }, [
                  ...
                  unref(isAdminUser) && unref(perms).canDo("customers", "list", "delete") ? (openBlock(), createBlock("button", {
                    key: 0,
                    onClick: withModifiers(($event) => confirmDelete(row), ["stop"]),
                    disabled: unref(deletingId) === row.id,
                    class: "btn-ghost text-xs py-1 px-2.5 text-red-400 hover:bg-red-500/10 disabled:opacity-40"
                  }, toDisplayString(unref(deletingId) === row.id ? "\u2026" : "Delete"), 9, ["onClick", "disabled"])) : createCommentVNode("", true)
                ])
              ];
            }
          }),
...
//# sourceMappingURL=index-CdSMw7w-.mjs.map
