  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        if (unref(form).method === "cash") {
          _push(`<div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Petty Cash Account *</label><select class="input-glass"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).cashAccountId) ? ssrLooseContain(unref(form).cashAccountId, "") : ssrLooseEqual(unref(form).cashAccountId, "")) ? " selected" : ""}>\u2014 Select cash box / account \u2014</option><!--[-->`);
          ssrRenderList(unref(pettyAccounts), (a) => {
            _push(`<option${ssrRenderAttr("value", a.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).cashAccountId) ? ssrLooseContain(unref(form).cashAccountId, a.id) : ssrLooseEqual(unref(form).cashAccountId, a.id)) ? " selected" : ""}>${ssrInterpolate(a.account_name)} (${ssrInterpolate(a.branch_name || "Head Office")}) </option>`);
          });
          _push(`<!--]--></select></div>`);
        } else {
          _push(`<!---->`);
...
//# sourceMappingURL=payment-DJwrtjxO.mjs.map
