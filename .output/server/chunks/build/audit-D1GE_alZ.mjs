    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        ssrRenderList(unref(logs), (log) => {
          ...
          if (log.record_type) {
            _push(`<span class="badge bg-purple-500/10 text-purple-400 border-purple-500/20 text-[10px]">${ssrInterpolate(log.record_type)}</span>`);
          } else {
            _push(`<!---->`);
          }
          if (log.reference_number) {
            _push(`<span class="text-[11px] text-gold-400/70 font-mono">${ssrInterpolate(log.reference_number)}</span>`);
          } else {
            _push(`<!---->`);
          }
          if (log.status) {
            _push(`<span class="${ssrRenderClass([
              ...
              log.status === "deleted" ? "bg-red-500/10 text-red-400" : log.status === "completed" ? "bg-emerald-500/10 text-emerald-400" : "bg-gray-500/10 text-gray-400"
            ])}">${ssrInterpolate(log.status)}</span>`);
          } else {
            _push(`<!---->`);
          }
          if (log.ip_address) {
...
//# sourceMappingURL=audit-D1GE_alZ.mjs.map
