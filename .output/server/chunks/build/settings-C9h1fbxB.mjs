  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(activeTab) === "delivery") {
        _push(`<div class="space-y-5"><div class="glass-card p-6 space-y-5"><div><h3 class="section-title">QR Delivery Verification</h3><p class="text-xs text-gray-500 mt-1"> Each credit sales invoice gets a unique QR code and 6-digit PIN. When scanned, the dispatcher or driver can confirm dispatch / delivery without logging into the ERP. </p></div><div class="flex items-start justify-between gap-4 py-4 border-t border-white/[0.06]"><div class="flex-1"><p class="text-sm font-semibold text-gray-200">Require Dispatch PIN <span class="text-[10px] font-normal text-emerald-400 ml-1.5">Active</span></p><p class="text-xs text-gray-500 mt-0.5"> Dispatcher must scan the invoice QR and enter the 6-digit PIN to mark an order as <span class="text-gray-300 font-mono text-[11px]">dispatched</span>. PIN is printed on the invoice footer and never expires. </p><p class="text-xs text-gray-600 mt-1">Status transition: <span class="text-purple-400 font-mono text-[11px]">ready_to_ship \u2192 dispatched</span></p></div><label class="relative inline-flex items-center cursor-pointer mt-0.5 shrink-0"><input${ssrIncludeBooleanAttr(Array.isArray(unref(deliverySettings).require_dispatch_pin) ? ssrLooseContain(unref(deliverySettings).require_dispatch_pin, null) : unref(deliverySettings).require_dispatch_pin) ? " checked" : ""} type="checkbox" class="sr-only peer"><div class="w-10 h-5 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-5 peer-checked:after:border-white after:content-[&#39;&#39;] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-white after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-gold-500"></div></label></div><div class="py-4 border-t border-white/[0.06]"><p class="text-sm font-semibold text-gray-200"> Final Delivery Confirmation \u2014 Authorized Staff <span class="text-[10px] font-normal text-emerald-400 ml-1.5">Active</span></p><p class="text-xs text-gray-500 mt-0.5"> Drivers do not confirm deliveries. When a <span class="text-gray-300 font-mono text-[11px]">dispatched</span> order&#39;s QR is scanned by a logged-in authorized user, they get a one-tap &quot;Confirm Final Delivery&quot; button that records the full delivery, posts the customer ledger entry and marks the order <span class="text-gray-300 font-mono text-[11px]">delivered</span>. </p><p class="text-xs text-gray-600 mt-1.5 mb-3"> Admins and superadmins are always authorized. Assign additional users below (e.g. the dispatch manager): </p><div class="rounded-xl bg-white/[0.03] border border-white/[0.07] divide-y divide-white/[0.05] max-h-64 overflow-y-auto"><!--[-->`);
        ssrRenderList(unref(assignableUsers), (u) => {
          _push(`<label class="flex items-center justify-between gap-3 px-4 py-2.5 cursor-pointer hover:bg-white/[0.03]"><div class="min-w-0"><p class="text-xs font-medium text-gray-200 truncate">${ssrInterpolate(u.display_name)}</p><p class="text-[10px] text-gray-500 truncate">${ssrInterpolate(u.email)} \xB7 ${ssrInterpolate(u.role)}</p></div><input type="checkbox" class="accent-amber-500 shrink-0"${ssrIncludeBooleanAttr(unref(deliverySettings).delivery_confirm_user_ids.includes(u.id)) ? " checked" : ""}></label>`);
        });
        _push(`<!--]-->`);
        if (!unref(assignableUsers).length) {
          _push(`<p class="px-4 py-3 text-xs text-gray-500">No other active users found.</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div><div class="rounded-xl p-4 bg-white/[0.03] border border-white/[0.07] space-y-2"><p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">How it works</p><ol class="text-xs text-gray-500 space-y-1.5 list-decimal list-inside"><li>Invoice is printed with a QR code and 6-digit PIN in the footer.</li><li>When order is <span class="text-gray-300 font-mono">ready_to_ship</span>, dispatcher scans the QR with any phone camera.</li><li>Public page opens \u2014 no login needed. Dispatcher enters the PIN.</li><li>Status updates to <span class="text-gray-300 font-mono">dispatched</span> automatically.</li><li>At handover, an authorized staff member (logged into the ERP on their phone) scans the same QR and taps <span class="text-gray-300">Confirm Final Delivery</span>.</li><li>Full delivery is recorded \u2014 ledger, journal entry and balances \u2014 and status becomes <span class="text-gray-300 font-mono">delivered</span>.</li><li>QR is rescanable \u2014 scanning again shows current status and audit trail.</li></ol></div>`);
...
//# sourceMappingURL=settings-C9h1fbxB.mjs.map
