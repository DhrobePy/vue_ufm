  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(categories), (cat) => {
        _push(`<div class="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/[0.07] hover:border-white/20 transition-all"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-xl flex items-center justify-center text-base" style="${ssrRenderStyle(`background: ${cat.color}20; border: 1px solid ${cat.color}40`)}">${ssrInterpolate(cat.icon)}</div><div><p class="text-sm font-semibold text-gray-200">${ssrInterpolate(cat.name)}</p><p class="text-xs text-gray-500 mt-0.5">${ssrInterpolate(cat.description)}</p></div></div><div class="flex items-center gap-4 text-right"><div><p class="text-xs text-gray-500">This month</p><p class="text-sm font-bold text-gray-200">\u09F3${ssrInterpolate(cat.monthlySpend.toLocaleString())}</p></div>`);
        if (cat.budget) {
...
//# sourceMappingURL=categories-DMlG7HSG.mjs.map
