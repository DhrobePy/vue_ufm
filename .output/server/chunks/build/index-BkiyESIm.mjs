...
import { defineComponent, withAsyncContext, computed, mergeProps, unref, withCtx, createTextVNode, createVNode, openBlock, createBlock, useSSRContext } from 'vue';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const receivedKg = computed(() => grns.value.reduce((s, g) => {
      var _a;
      return s + Number((_a = g.quantity_kg) != null ? _a : 0);
    }, 0));
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        ssrRenderList(unref(grns), (grn) => {
          _push(`<div class="flex items-center justify-between p-3 rounded-xl bg-white/[0.03] border border-white/[0.07] text-xs"><div><p class="font-mono text-gold-400/80">${ssrInterpolate(grn.grn_number)}</p><p class="text-gray-500 mt-0.5">${ssrInterpolate(grn.grn_date)} \xB7 ${ssrInterpolate(Number(grn.quantity_kg).toLocaleString())} kg `);
          if (grn.grade) {
            _push(`<span> \xB7 Grade ${ssrInterpolate(grn.grade)}</span>`);
          } else {
            _push(`<!---->`);
          }
          if (grn.moisture_pct) {
            _push(`<span> \xB7 Moisture ${ssrInterpolate(grn.moisture_pct)}%</span>`);
          } else {
            _push(`<!---->`);
          }
        ...
        if (unref(payments).length) {
          ...
          ssrRenderList(unref(payments), (pmt) => {
            _push(`<div class="flex items-center justify-between p-3 rounded-xl bg-white/[0.03] border border-white/[0.07] text-xs"><div><p class="font-semibold text-gray-200">\u09F3${ssrInterpolate(Number(pmt.amount).toLocaleString())}</p><p class="text-gray-500 mt-0.5">${ssrInterpolate(pmt.payment_date)} \xB7 ${ssrInterpolate(pmt.payment_mode || pmt.payment_method || "\u2014")} `);
            if (pmt.bank_name) {
            ...
            _push(`</p></div>`);
            _push(ssrRenderComponent(_component_UiStatusBadge, {
              status: pmt.payment_status || "approved"
            }, null, _parent));
            _push(`</div>`);
          });
          _push(`<!--]--></div>`);
        ...
        _push(`<button class="btn-ghost text-xs w-full justify-start gap-2 text-red-400 hover:text-red-300 hover:border-red-500/30"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M18.364 5.636l-12.728 12.728"></path><path stroke-linecap="round" stroke-linejoin="round" d="M5.636 5.636l12.728 12.728"></path></svg> Cancel PO </button></div></div></div><!--]-->`);
      }
      _push(`</div>`);
    };
  }
...
//# sourceMappingURL=index-BkiyESIm.mjs.map
