...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const unitLabel = computed(() => {
      var _a;
      const u = (_a = selectedPO.value) == null ? void 0 : _a.commodity_unit;
      return !u || u === "MT" ? "KG" : u;
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(openPOs), (po) => {
        var _a2, _b2;
        _push(`<option${ssrRenderAttr("value", po.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).po_id) ? ssrLooseContain(unref(form).po_id, po.id) : ssrLooseEqual(unref(form).po_id, po.id)) ? " selected" : ""}> PO #${ssrInterpolate(po.po_number)} \u2014 ${ssrInterpolate(po.supplier_name)} \u2014 ${ssrInterpolate((_a2 = po.commodity_name) != null ? _a2 : "Wheat")} (${ssrInterpolate(po.wheat_origin)}) (Pending: ${ssrInterpolate(Number(po.qty_yet_to_receive).toLocaleString())} ${ssrInterpolate((_b2 = po.commodity_unit) != null ? _b2 : "KG")}) </option>`);
      });
      _push(`<!--]--></select></div>`);
      if (unref(selectedPO)) {
        _push(`<div class="rounded-xl bg-blue-500/[0.05] border border-blue-500/20 p-4 text-xs space-y-2"><div class="grid grid-cols-2 gap-2"><div class="space-y-1.5"><div class="flex justify-between"><span class="text-gray-500">Commodity</span><span class="text-gray-200">${ssrInterpolate((_a = unref(selectedPO).commodity_name) != null ? _a : "Wheat")}</span></div><div class="flex justify-between"><span class="text-gray-500">Supplier</span><span class="text-gray-200">${ssrInterpolate(unref(selectedPO).supplier_name)}</span></div><div class="flex justify-between"><span class="text-gray-500">Origin</span><span class="text-gray-200">${ssrInterpolate(unref(selectedPO).wheat_origin || "\u2014")}</span></div><div class="flex justify-between"><span class="text-gray-500">Unit Price</span><span class="text-gray-200 font-mono">\u09F3${ssrInterpolate(Number(unref(selectedPO).unit_price_per_kg).toLocaleString())}/${ssrInterpolate(unref(unitLabel))}</span></div></div><div class="space-y-1.5"><div class="flex justify-between"><span class="text-gray-500">Ordered</span><span class="text-gray-200 font-mono">${ssrInterpolate(Number(unref(selectedPO).quantity_kg).toLocaleString())} ${ssrInterpolate(unref(unitLabel))}</span></div><div class="flex justify-between"><span class="text-gray-500">Already Received</span><span class="text-gray-200 font-mono">${ssrInterpolate(Number((_b = unref(selectedPO).total_received_qty) != null ? _b : 0).toLocaleString())} ${ssrInterpolate(unref(unitLabel))}</span></div><div class="flex justify-between"><span class="text-gray-500">Yet to Receive</span><span class="${ssrRenderClass([Number(unref(selectedPO).qty_yet_to_receive) > 0 ? "text-orange-300" : "text-emerald-400", "font-mono font-semibold"])}">${ssrInterpolate(Number(unref(selectedPO).qty_yet_to_receive).toLocaleString())} ${ssrInterpolate(unref(unitLabel))}</span></div></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider"> GRN Date <span class="text-red-500">*</span></label><input${ssrRenderAttr("value", unref(form).grn_date)} type="date" class="input-glass"${ssrRenderAttr("max", unref(today))}></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Truck Number</label><input${ssrRenderAttr("value", unref(form).truck_number)} type="text" class="input-glass" placeholder="e.g., 1234" maxlength="20"></div></div><div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider"> Quantity Received (${ssrInterpolate(unref(unitLabel))}) <span class="text-red-500">*</span></label><input${ssrRenderAttr("value", unref(form).quantity_received_kg)} type="number" step="0.01" min="0.01" class="input-glass font-mono text-lg"></div><div class="space-y-1.5"><label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Expected Quantity (${ssrInterpolate(unref(unitLabel))})</label><input${ssrRenderAttr("value", unref(form).expected_quantity)} type="number" step="0.01" min="0" class="input-glass font-mono"><p class="text-[10px] text-gray-600">Optional \u2014 for variance tracking (as per truck challan)</p></div></div>`);
      if (unref(showVariance)) {
        _push(`<div class="${ssrRenderClass([unref(varianceBg), "rounded-xl px-4 py-3 text-xs font-semibold border"])}"><span class="font-bold">Variance:</span> ${ssrInterpolate(unref(varianceKg) > 0 ? "+" : "")}${ssrInterpolate(unref(varianceKg).toFixed(2))} ${ssrInterpolate(unref(unitLabel))} (${ssrInterpolate(unref(varianceKg) > 0 ? "+" : "")}${ssrInterpolate(unref(variancePct).toFixed(2))}%) </div>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(overDelivery).show) {
        _push(`<div class="rounded-xl p-4 bg-orange-500/[0.08] border-2 border-orange-400/40 space-y-3"><div class="flex items-start gap-3"><span class="text-orange-400 text-lg">\u26A0</span><div class="flex-1 space-y-1"><p class="font-semibold text-orange-300 text-sm">Over-Delivery Detected</p><p class="text-xs text-orange-200/80"> Remaining on PO: <strong>${ssrInterpolate(unref(overDelivery).pending.toFixed(2))}</strong> ${ssrInterpolate(unref(unitLabel))} | Recording: <strong>${ssrInterpolate(unref(form).quantity_received_kg.toFixed(2))}</strong> ${ssrInterpolate(unref(unitLabel))} | Excess: <strong class="text-red-400">${ssrInterpolate(unref(overDelivery).excess.toFixed(2))}</strong> ${ssrInterpolate(unref(unitLabel))}</p><p class="text-xs text-orange-200/70">How would you like to handle the excess?</p></div></div><div class="flex flex-wrap gap-2"><button type="button" class="${ssrRenderClass([unref(form).over_delivery_action === "accept_with_dan" ? "bg-orange-500 text-white" : "bg-white/[0.08] text-orange-300 hover:bg-orange-500/30", "px-3 py-2 rounded-lg text-xs font-semibold transition-colors"])}"> \u{1F4C4} Accept All + Auto-Draft Debit Note (DAN) </button><button type="button" class="${ssrRenderClass([unref(form).over_delivery_action === "as_is" ? "bg-gray-600 text-white" : "bg-white/[0.08] text-gray-300 hover:bg-gray-600/40", "px-3 py-2 rounded-lg text-xs font-semibold transition-colors"])}"> \u2714 Record As-Is (handle manually later) </button></div>`);
...
//# sourceMappingURL=create-CtEpuToA.mjs.map
