...
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderTeleport, ssrRenderStyle, ssrInterpolate, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderAttr, ssrRenderClass } from 'vue/server-renderer';
      ...
      if (unref(req)) {
        ...
        ssrRenderTeleport(_push, (_push2) => {
          if (unref(completeModal)) {
            _push2(`<div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="${ssrRenderStyle({ "background": "rgba(0,0,0,0.7)", "backdrop-filter": "blur(4px)" })}"><div class="glass-card p-6 w-full max-w-sm space-y-4"><h3 class="text-sm font-semibold text-gray-200">Mark Completed</h3><p class="text-xs text-gray-500"> Total cost <span class="text-gold-400 font-semibold">\u09F3${ssrInterpolate(Number(unref(req).total_cost || 0).toLocaleString())}</span> will be posted to the ledger as a Vehicle Maintenance expense. </p><div><label class="field-label">Paid From *</label><select class="field-input w-full"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(payMethod)) ? ssrLooseContain(unref(payMethod), "") : ssrLooseEqual(unref(payMethod), "")) ? " selected" : ""}>Select\u2026</option><option value="cash"${ssrIncludeBooleanAttr(Array.isArray(unref(payMethod)) ? ssrLooseContain(unref(payMethod), "cash") : ssrLooseEqual(unref(payMethod), "cash")) ? " selected" : ""}>Petty Cash</option><option value="bank"${ssrIncludeBooleanAttr(Array.isArray(unref(payMethod)) ? ssrLooseContain(unref(payMethod), "bank") : ssrLooseEqual(unref(payMethod), "bank")) ? " selected" : ""}>Bank</option></select></div>`);
            if (unref(payMethod) === "cash") {
              _push2(`<div><label class="field-label">Petty Cash Account</label><select class="field-input w-full"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(payCashAccount)) ? ssrLooseContain(unref(payCashAccount), "") : ssrLooseEqual(unref(payCashAccount), "")) ? " selected" : ""}>\u2014 Select \u2014</option><!--[-->`);
              ssrRenderList(unref(pettyCashAccounts), (a) => {
                _push2(`<option${ssrRenderAttr("value", a.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(payCashAccount)) ? ssrLooseContain(unref(payCashAccount), a.id) : ssrLooseEqual(unref(payCashAccount), a.id)) ? " selected" : ""}>${ssrInterpolate(a.account_name)} (\u09F3${ssrInterpolate(Number(a.current_balance).toLocaleString())})</option>`);
              });
              _push2(`<!--]--></select></div>`);
            } else {
              _push2(`<!---->`);
            }
            if (unref(payMethod) === "bank") {
              _push2(`<div><label class="field-label">Bank Account</label><select class="field-input w-full"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(payBankAccount)) ? ssrLooseContain(unref(payBankAccount), "") : ssrLooseEqual(unref(payBankAccount), "")) ? " selected" : ""}>\u2014 Select \u2014</option><!--[-->`);
              ssrRenderList(unref(bankAccounts), (a) => {
                _push2(`<option${ssrRenderAttr("value", a.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(payBankAccount)) ? ssrLooseContain(unref(payBankAccount), a.id) : ssrLooseEqual(unref(payBankAccount), a.id)) ? " selected" : ""}>${ssrInterpolate(a.bank_name)} \u2014 ${ssrInterpolate(a.account_name)}</option>`);
              });
              _push2(`<!--]--></select></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="flex gap-2 pt-1"><button class="btn-ghost text-xs flex-1 justify-center">Cancel</button><button${ssrIncludeBooleanAttr(!unref(payMethod) || unref(payMethod) === "cash" && !unref(payCashAccount) || unref(payMethod) === "bank" && !unref(payBankAccount)) ? " disabled" : ""} class="btn-gold text-xs flex-1 justify-center disabled:opacity-40"> Complete &amp; Post </button></div></div></div>`);
          } else {
            _push2(`<!---->`);
          }
        }, "body", false, _parent);
...
//# sourceMappingURL=_id_-C30Die9W.mjs.map
