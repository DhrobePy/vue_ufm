...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const initialAccount = route.query.account ? Number(route.query.account) : "";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(applied).account) {
        ...
        _push(`<!--[-->`);
        if (!((_a = unref(data)) == null ? void 0 : _a.opening_balance_configured)) {
          _push(`<div class="glass-card border border-yellow-500/20 bg-yellow-500/[0.04] p-4 flex items-start gap-3"><span class="text-yellow-400 text-lg mt-0.5">\u26A0</span><div class="flex-1 text-xs"><p class="text-yellow-300 font-semibold mb-0.5">Opening balance not configured for this account</p><p class="text-yellow-400/70"> Balances shown are net GL movement within this period only (starting from \u09F30). To see accurate running balances, set the opening balance \u2014 the real-world bank balance at the point when GL tracking began for this account. </p></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="grid grid-cols-2 lg:grid-cols-4 gap-4"><div class="glass-card p-4"><p class="text-xs text-gray-500 mb-1">${ssrInterpolate(((_b = unref(data)) == null ? void 0 : _b.opening_balance_configured) ? "Opening Balance" : "Opening Balance")}</p><p class="${ssrRenderClass([unref(openingBalance) >= 0 ? "text-gray-100" : "text-red-400", "text-xl font-bold"])}"> \u09F3${ssrInterpolate(unref(openingBalance).toLocaleString())}</p><p class="${ssrRenderClass([((_c = unref(data)) == null ? void 0 : _c.opening_balance_configured) ? "text-gray-600" : "text-yellow-600", "text-[10px] mt-0.5"])}">${ssrInterpolate(((_d = unref(data)) == null ? void 0 : _d.opening_balance_configured) ? `as of ${unref(applied).from || "account start"}` : "\u26A0 not configured \u2014 set to calibrate")}</p></div><div class="glass-card p-4"><p class="text-xs text-gray-500 mb-1">Total Credits (In)</p><p class="text-xl font-bold text-emerald-400">\u09F3${ssrInterpolate(unref(totalCredits).toLocaleString())}</p><p class="text-[10px] text-gray-600 mt-0.5">money received</p></div><div class="glass-card p-4"><p class="text-xs text-gray-500 mb-1">Total Debits (Out)</p><p class="text-xl font-bold text-red-400">\u09F3${ssrInterpolate(unref(totalDebits).toLocaleString())}</p><p class="text-[10px] text-gray-600 mt-0.5">money paid out</p></div><div class="glass-card p-4"><p class="text-xs text-gray-500 mb-1">Closing Balance</p><p class="${ssrRenderClass([unref(closingBalance) >= 0 ? "text-gold-400" : "text-red-400", "text-xl font-bold"])}"> \u09F3${ssrInterpolate(unref(closingBalance).toLocaleString())}</p><p class="${ssrRenderClass([((_e = unref(data)) == null ? void 0 : _e.opening_balance_configured) ? "text-gray-600" : "text-yellow-600", "text-[10px] mt-0.5"])}">${ssrInterpolate(((_f = unref(data)) == null ? void 0 : _f.opening_balance_configured) ? `as of ${unref(applied).to || "today"}` : "\u26A0 net movement only")}</p></div></div><div id="statement-print" class="glass-card p-5"><div class="flex items-center justify-between mb-5"><div><h3 class="section-title">${ssrInterpolate(unref(accountLabel))}</h3>`);
        if ((_h = (_g = unref(data)) == null ? void 0 : _g.bankAccount) == null ? void 0 : _h.account_number) {
          _push(`<p class="text-xs text-gray-500 mt-0.5"> Account No: ${ssrInterpolate(unref(data).bankAccount.account_number)}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div><div class="text-right text-xs text-gray-500 space-y-0.5"><p>${ssrInterpolate(unref(applied).from || "All dates")} \u2192 ${ssrInterpolate(unref(applied).to || "present")}</p><p class="font-mono">${ssrInterpolate(unref(filteredTxns).length)} entries</p></div></div><div class="overflow-x-auto"><table class="w-full text-xs min-w-[680px]"><thead><tr class="border-b border-white/[0.08]"><th class="pb-2.5 px-3 text-left text-gray-600 font-semibold uppercase tracking-wider w-[100px]">Date</th><th class="pb-2.5 px-3 text-left text-gray-600 font-semibold uppercase tracking-wider">Description</th><th class="pb-2.5 px-3 text-center text-gray-600 font-semibold uppercase tracking-wider w-[90px]">Source</th><th class="pb-2.5 px-3 text-left text-gray-600 font-semibold uppercase tracking-wider w-[72px]">Ref</th><th class="pb-2.5 px-3 text-right text-red-400/70 font-semibold uppercase tracking-wider w-[120px]">Debit (Out)</th><th class="pb-2.5 px-3 text-right text-emerald-400/70 font-semibold uppercase tracking-wider w-[120px]">Credit (In)</th><th class="pb-2.5 px-3 text-right text-gray-400/70 font-semibold uppercase tracking-wider w-[130px]">Balance</th></tr></thead><tbody class="divide-y divide-white/[0.03]"><tr class="bg-blue-500/[0.04] border-b border-blue-500/10"><td class="py-2 px-3 text-blue-400/60 font-mono text-[11px] whitespace-nowrap">${ssrInterpolate(unref(applied).from || "Account Start")}</td><td colspan="4" class="py-2 px-3 text-blue-400/70 font-medium"> \u25C6 Opening Balance </td><td class="py-2 px-3"></td><td class="py-2 px-3 text-right font-bold text-blue-300 font-mono"> \u09F3${ssrInterpolate(unref(openingBalance).toLocaleString())}</td></tr>`);
        ...
        ssrRenderList(unref(filteredTxns), (tx) => {
          ...
          if (tx.is_reversed) {
            _push(`<span class="text-orange-400/60 text-[10px] block mt-0.5">\u27F2 Reversed \u2014 cancelled by paired entry</span>`);
          } else {
            _push(`<!---->`);
          }
          ...
          if (tx.debit_out > 0) {
            _push(`<span class="${ssrRenderClass(tx.is_reversed ? "text-red-400/30 line-through" : "text-red-400")}"> \u09F3${ssrInterpolate(Number(tx.debit_out).toLocaleString())}</span>`);
          } else {
            _push(`<span class="text-gray-700">\u2014</span>`);
          }
          _push(`</td><td class="py-2.5 px-3 text-right font-mono">`);
          if (tx.credit_in > 0) {
            _push(`<span class="${ssrRenderClass(tx.is_reversed ? "text-emerald-400/30 line-through" : "text-emerald-400")}"> \u09F3${ssrInterpolate(Number(tx.credit_in).toLocaleString())}</span>`);
          } else {
            _push(`<span class="text-gray-700">\u2014</span>`);
          }
        ...
        _push(`<!--]--><tr class="bg-gold-500/[0.04] border-t border-gold-500/10"><td class="py-2 px-3 text-gold-400/60 font-mono text-[11px] whitespace-nowrap">${ssrInterpolate(unref(applied).to || unref(today))}</td><td colspan="4" class="py-2 px-3 text-gold-400/70 font-medium"> \u2726 Closing Balance </td><td class="py-2 px-3"></td><td class="py-2 px-3 text-right font-bold text-gold-300 font-mono"> \u09F3${ssrInterpolate(unref(closingBalance).toLocaleString())}</td></tr></tbody>`);
        if (unref(filteredTxns).length) {
        ...
        _push(`</table></div>`);
        if (((_j = (_i = unref(data)) == null ? void 0 : _i.transactions) != null ? _j : []).length >= 2e3) {
...
//# sourceMappingURL=statement-Bw-CL1rJ.mjs.map
