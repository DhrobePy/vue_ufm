    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(applied).account) {
        ...
        ssrRenderList(unref(filteredTxns), (tx) => {
          ...
          if (!tx.is_reversed && tx.debit_out > 0) {
            _push(`<span class="text-red-400"> \u09F3${ssrInterpolate(Number(tx.debit_out).toLocaleString())}</span>`);
          } else {
            _push(`<span class="text-gray-700">\u2014</span>`);
          }
          _push(`</td><td class="py-2.5 px-3 text-right font-mono">`);
          if (!tx.is_reversed && tx.credit_in > 0) {
            _push(`<span class="text-emerald-400"> \u09F3${ssrInterpolate(Number(tx.credit_in).toLocaleString())}</span>`);
          } else {
            _push(`<span class="text-gray-700">\u2014</span>`);
          }
          _push(`</td><td class="py-2.5 px-3 text-right font-mono font-semibold">`);
          if (tx.is_reversed) {
            _push(`<span class="text-gray-700 text-[11px] italic">reversed</span>`);
          } else {
            _push(`<span class="${ssrRenderClass(Number(tx.balance) >= 0 ? "text-gray-200" : "text-red-400")}"> \u09F3${ssrInterpolate(Number(tx.balance).toLocaleString())}</span>`);
          }
          _push(`</td></tr>`);
        });
...
//# sourceMappingURL=statement-CkMwBN6u.mjs.map
