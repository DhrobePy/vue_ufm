    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        ssrRenderList(unref(orders), (order) => {
          ...
          if (unref(perms).canDo("credit_sales", "approve", creditPct(order) > 80 ? "escalate" : "approve")) {
            ...
            ])}"${ssrIncludeBooleanAttr(unref(acting) === order.id) ? " disabled" : ""}>${ssrInterpolate(unref(acting) === order.id ? "\u2026" : creditPct(order) > 80 ? "Escalate" : "Approve")}</button>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div></div>`);
...
//# sourceMappingURL=approve-DqUEgnB2.mjs.map
