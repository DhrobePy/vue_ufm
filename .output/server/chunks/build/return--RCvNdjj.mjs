...
import { j as useRoute, o as useUserSession } from './server.mjs';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(orderPending)) {
        ...
        ssrRenderList(unref(returnItems), (item, i) => {
          var _a2;
          _push(`<tr><td class="py-3 px-2"><p class="text-gray-200 font-medium">${ssrInterpolate(item.product_name || "\u2014")}</p><p class="text-gray-600 text-[10px]">${ssrInterpolate((_a2 = item.weight_variant) != null ? _a2 : "")}</p></td><td class="py-3 px-2 text-right text-gray-400">${ssrInterpolate(Number(item.quantity).toFixed(0))}</td><td class="py-3 px-2 text-right text-gray-400">\u09F3${ssrInterpolate(Number(item.unit_price).toLocaleString())}</td><td class="py-3 px-2 text-right"><input${ssrRenderAttr("value", unref(returnItems)[i].return_qty)} type="number" min="0"${ssrRenderAttr("max", item.quantity)} step="1" class="w-20 bg-white/[0.06] border border-white/[0.10] rounded-lg px-2 py-1.5 text-right text-xs text-gray-200 outline-none focus:border-gold-500/50 focus:ring-1 focus:ring-gold-500/30"></td><td class="${ssrRenderClass([item.return_qty > 0 ? "text-red-400" : "text-gray-600", "py-3 px-2 text-right font-mono"])}">${ssrInterpolate(item.return_qty > 0 ? "\u09F3" + (item.return_qty * Number(item.unit_price)).toLocaleString() : "\u2014")}</td></tr>`);
        });
        _push(`<!--]-->`);
        ...
        _push(`</div></div><div class="glass-card p-5 space-y-2"><h3 class="text-sm font-semibold text-gray-300">Approval</h3><div class="${ssrRenderClass([unref(isAdmin) ? "bg-emerald-500/10 border border-emerald-500/20 text-emerald-400" : "bg-amber-500/10 border border-amber-500/20 text-amber-400", "rounded-xl p-3 text-xs"])}"><p class="font-semibold mb-1">${ssrInterpolate(unref(isAdmin) ? "\u2713 Auto-Approved" : "\u23F3 Pending Approval")}</p><p class="opacity-80 leading-snug">${ssrInterpolate(unref(isAdmin) ? "As admin, the return will be approved immediately and balance adjusted." : "Return will be submitted as pending. An admin must approve it before the balance is adjusted.")}</p></div></div><div class="glass-card p-5 space-y-3"><h3 class="text-sm font-semibold text-gray-300">Previous Returns `);
...
//# sourceMappingURL=return--RCvNdjj.mjs.map
