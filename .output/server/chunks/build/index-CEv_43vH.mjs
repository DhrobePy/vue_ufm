...
import { defineComponent, ref, computed, withAsyncContext, mergeProps, unref, withCtx, createTextVNode, createVNode, openBlock, createBlock, useSSRContext } from 'vue';
...
import { j as useRoute, o as useUserSession, n as navigateTo } from './server.mjs';
      ...
      if (unref(pending)) {
        ...
        if (unref(payments).length) {
          ...
          ssrRenderList(unref(payments), (pmt) => {
            var _a;
            _push(`<div class="flex items-center justify-between p-3 rounded-xl bg-white/[0.03] border border-white/[0.07] text-xs"><div><p class="font-semibold text-gray-200">\u09F3${ssrInterpolate(Number((_a = pmt.amount_paid) != null ? _a : 0).toLocaleString())}</p><p class="text-gray-500 mt-0.5">${ssrInterpolate(fmtDate(pmt.payment_date))} \xB7 ${ssrInterpolate(pmt.payment_mode || pmt.payment_method || "\u2014")} `);
            if (pmt.bank_name) {
...
//# sourceMappingURL=index-CEv_43vH.mjs.map
