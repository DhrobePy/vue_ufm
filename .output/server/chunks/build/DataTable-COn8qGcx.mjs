  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.columns, (col) => {
        ...
        if (col.sortable) {
          _push(`<span class="flex flex-col gap-0.5"><svg class="${ssrRenderClass(["w-2.5 h-2.5 transition-colors", unref(sortKey) !== col.key || unref(sortDir) !== "asc" ? "text-gray-700" : ""])}" style="${ssrRenderStyle(unref(sortKey) === col.key && unref(sortDir) === "asc" ? "color: var(--accent-from)" : "")}" viewBox="0 0 10 6" fill="currentColor"><path d="M0 6l5-6 5 6H0z"></path></svg><svg class="${ssrRenderClass(["w-2.5 h-2.5 transition-colors", unref(sortKey) !== col.key || unref(sortDir) !== "desc" ? "text-gray-700" : ""])}" style="${ssrRenderStyle(unref(sortKey) === col.key && unref(sortDir) === "desc" ? "color: var(--accent-from)" : "")}" viewBox="0 0 10 6" fill="currentColor"><path d="M0 0l5 6 5-6H0z"></path></svg></span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</span></th>`);
      ...
      if (!__props.loading && unref(filteredRows).length > unref(perPage)) {
        ...
        ssrRenderList(unref(pageNumbers), (p) => {
          _push(`<button class="${ssrRenderClass([
            ...
            p !== unref(currentPage) && "text-gray-500 hover:text-gray-200 hover:bg-white/[0.07]"
          ])}" style="${ssrRenderStyle(p === unref(currentPage) ? "background:rgb(var(--accent)/0.15);color:var(--accent-from);border:1px solid rgb(var(--accent)/0.25)" : "")}">${ssrInterpolate(p)}</button>`);
        });
...
//# sourceMappingURL=DataTable-COn8qGcx.mjs.map
