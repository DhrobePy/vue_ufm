...
export default defineEventHandler(async (event) => {
  ...
  const conditions: string[] = ["o.order_status = 'Completed'"]
  const params: any[]        = []

  if (from)     { conditions.push("DATE(o.order_date) >= ?"); params.push(from) }
  if (to)       { conditions.push("DATE(o.order_date) <= ?"); params.push(to) }
  ...
  const grandTotal = productBreakdown.reduce((s: number, r: any) => s + Number(r.total), 0)
  const productPct = productBreakdown.map((r: any) => ({
    name:   r.product_name,
  ...
  const maxDay = Math.max(...days.map((d: any) => Number(d.day_total)), 1)
  const dayBars = days.map((d: any) => ({
