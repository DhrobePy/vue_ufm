...
/** Credit orders CSV — optional ?status= & ?from= & ?to= & ?customer_id=. */
export default defineEventHandler(async (event) => {
  ...
  const lines = rows.map((r: any) => [
    r.order_number, String(r.order_date).slice(0, 10), r.customer, r.branch,
    r.status, r.priority, r.delivery_type,
    r.subtotal, r.mini_truck_surcharge, r.total_amount,
    r.advance_paid, r.amount_paid, r.balance_due,
  ].map(csvCell).join(','))

  setHeader(event, 'Content-Type', 'text/csv; charset=utf-8')
  setHeader(event, 'Content-Disposition',
    `attachment; filename="credit-orders-${new Date().toISOString().slice(0, 10)}.csv"`)
