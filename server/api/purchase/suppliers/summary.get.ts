...
export default defineEventHandler(async (event) => {
  ...
  // Check if adjustment notes table exists
  ...
  // Build the main supplier summary query
  const adjDebitSub = hasAdjNotes
    ? `COALESCE((
  ...
  const adjCreditSub = hasAdjNotes
    ? `COALESCE((
        SELECT SUM(pan.amount)
        FROM purchase_adjustment_notes pan
        JOIN purchase_orders_adnan po2 ON pan.purchase_order_id = po2.id
  ...
  // Compute derived fields per supplier
  const suppliers = rows.map((s: any) => {
    const adjDebit   = Number(s.total_adj_debit ?? 0)
    const adjCredit  = Number(s.total_adj_credit ?? 0)
    const receivable = Number(s.total_receivable_value ?? 0)
  ...
  // Grand totals
  const totals = suppliers.reduce((acc: any, s: any) => {
    acc.suppliers++
