...
export default defineEventHandler(async (event) => {
  ...
  const supplier_id = (q.supplier_id as string) || ''
  const wheat_origin= (q.wheat_origin as string) || ''
  const grn_status  = (q.grn_status  as string) || ''
  const unload_point= (q.unload_point as string) || ''
  const truck_number= (q.truck_number as string) || ''
  ...
  if (date_from)    { where.push('g.grn_date >= ?');                              params.push(date_from) }
  if (date_to)      { where.push('g.grn_date <= ?');                              params.push(date_to) }
  if (supplier_id)  { where.push('g.supplier_id = ?');                            params.push(supplier_id) }
  if (wheat_origin) { where.push('po.wheat_origin = ?');                          params.push(wheat_origin) }
  if (grn_status)   { where.push('g.grn_status = ?');                             params.push(grn_status) }
  if (unload_point) { where.push('g.unload_point_name = ?');                      params.push(unload_point) }
  if (truck_number) { where.push('g.truck_number LIKE ?');                        params.push(`%${truck_number}%`) }
  if (search)       {
  ...
  // Filter dropdowns — distinct values
