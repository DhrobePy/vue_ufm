...
/** DELETE /api/loans/repayments/:repayId — reverse one loan repayment (recycle-backed). */
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const [[rep]] = await conn.query<any>(
    ...
    const batchId = await recycleBegin(conn, {
      ...
      customerId: rep.customer_id ?? null, userId, userName,
    })

    // Restore loan balances (+ re-open a closed loan)
    ...
    // Cash: money back out of the box
    if (rep.cash_account_id) {
      ...
      const [pcTxns] = await conn.query<any>(
        `SELECT id FROM branch_petty_cash_transactions WHERE reference_type = 'loan_repayment' AND reference_id = ?`, [repayId],
      )
