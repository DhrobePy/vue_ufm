...
export default defineEventHandler(async (event) => {
  ...
  const tripExpenses = await query<any>(
  ...
  // Monthly fuel cost
  const fuelCost = await query<any>(
  ...
  const maintCost = await query<any>(
  ...
  const toMap = (rows: any[]) => Object.fromEntries(rows.map((r: any) => [r.month, Number(r.amount)]))
  ...
  // Available years
  const [minRow] = await query<any>(`SELECT MIN(YEAR(trip_date)) AS min_year FROM trips`, [])
  const minYear  = Number(minRow?.min_year || year)
  const years    = Array.from({ length: new Date().getFullYear() - minYear + 2 }, (_, i) => minYear + i)
