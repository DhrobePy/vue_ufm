...
export default defineEventHandler(async (event) => {
  ...
  const from = (q.from as string) || new Date(new Date().getFullYear(), 0, 1).toISOString().slice(0, 10)
  ...
  const totalCost = byVehicle.reduce((s: number, r: any) => s + Number(r.total_cost || 0), 0)
