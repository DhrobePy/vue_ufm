...
export default defineEventHandler(async (event) => {
  ...
  // Compute settlement summary
  const totalAdvances  = (advances as any[]).reduce((s, a) => s + Number(a.amount), 0)
  const totalExpenses  = (expenses as any[]).reduce((s, e) => s + Number(e.amount), 0)
  const settlement = {
    revenue:         Number((trip as any).trip_charge),
    total_advance:   totalAdvances,
    total_expense:   totalExpenses,
    final_balance:   Number((trip as any).trip_charge) - totalAdvances - totalExpenses,
  }
