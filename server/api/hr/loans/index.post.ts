...
export default defineEventHandler(async (event) => {
  ...
  if (action === 'create') {
    ...
    const startDate     = loan_date || new Date().toISOString().slice(0, 10)
    ...
    // Pre-create all installment schedule rows
  ...
  if (action === 'pay_installment') {
    ...
    const inst = await queryOne('SELECT * FROM hr_loan_installments WHERE id = ?', [installment_id]) as any
    if (inst) {
      const loan = await queryOne('SELECT * FROM hr_loans WHERE id = ?', [inst.loan_id]) as any
