...
export default defineEventHandler(async (event) => {
  ...
  if (action === 'prepare') {
    ...
    const periodMonth = Number(pay_period_start.slice(5, 7))   // M
    const periodYear  = Number(pay_period_start.slice(0, 4))   // YYYY
    const db = getDb()
    ...
    for (const emp of activeEmployees as any[]) {
      try {
        ...
        `, [emp.id, pay_period_start, pay_period_end]) as any[]
        ...
        `, [emp.id, periodMonth, periodYear]) as any[]
        ...
        // Insert payroll_details breakdown
        await db.query(
          `INSERT INTO hr_payroll_details
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [payrollId, basicSalary, houseAllowance, transportAllowance, medicalAllowance,
           otherAllowances, grossSalary, daysInMonth, absentDays, dailyRate,
           absenceDeduction, advanceDeduction, loanDeduction,
           providentFund, taxDeduction, otherDeductions, totalDeductions, netSalary]
        )
  ...
  // ── UPDATE SINGLE RECORD (admin edit during review) ───────
  if (action === 'update_record') {
    const { payroll_id, gross_salary, absence_deduction, advance_deduction,
            loan_deduction, provident_fund, tax_deduction, other_deductions } = body
  ...
  if (action === 'reject') {
    ...
    if (!Array.isArray(payroll_ids) || !payroll_ids.length)
      throw createError({ statusCode: 400, statusMessage: 'payroll_ids required' })
    const placeholders = (payroll_ids as number[]).map(() => '?').join(',')
  ...
  if (action === 'pay') {
    ...
    for (const pid of payroll_ids as number[]) {
      ...
      // Mark due loan installments as paid
      await db.query(
