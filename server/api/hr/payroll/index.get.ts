...
export default defineEventHandler(async (event) => {
  ...
  const view       = (q.view as string) || 'history'  // history | pending | payslip | approved
  const payrollId  = q.payrollId  ? Number(q.payrollId)  : null
  const employeeId = q.employeeId ? Number(q.employeeId) : null
  const period     = q.period as string | undefined  // YYYY-MM
  ...
  if (period)     { sql += ' AND DATE_FORMAT(p.pay_period_start, \'%Y-%m\') = ?'; params.push(period) }
