...
export default defineEventHandler(async (event) => {
  ...
  if (action === 'create') {
    const {
      ...
      branch_id, bank_name, bank_account, bank_branch, nid, dob, gender, blood_group,
      device_pin, emergency_contact
    } = body
    ...
    const [res] = await getDb().query(
      ...
      [first_name, last_name || '', email || null, phone || null, address || null,
       position_id || null, hire_date, Number(base_salary) || 0, status || 'active',
       branch_id || 1, bank_name || '', bank_account || '', bank_branch || '',
       nid || null, dob || null, gender || null, blood_group || null, device_pin || null, emergency_contact || null]
    ) as any
  ...
  if (action === 'update') {
    const {
      id, first_name, last_name, email, phone, address, position_id, hire_date, base_salary, status,
      branch_id, bank_name, bank_account, bank_branch, nid, dob, gender, blood_group,
      device_pin, emergency_contact
    } = body
    ...
    await query(
      ...
      [first_name, last_name || '', email || null, phone || null, address || null,
       position_id || null, hire_date, Number(base_salary) || 0, status || 'active',
       branch_id || 1, bank_name || '', bank_account || '', bank_branch || '',
       nid || null, dob || null, gender || null, blood_group || null, device_pin || null, emergency_contact || null, id]
    )
  ...
  if (action === 'create_dept') {
    ...
    const [res] = await getDb().query('INSERT INTO hr_departments (name) VALUES (?)', [name]) as any
