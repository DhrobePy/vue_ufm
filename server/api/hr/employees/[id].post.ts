...
export default defineEventHandler(async (event) => {
  ...
  if (action === 'update_employee') {
    ...
    await query(`UPDATE hr_employees SET
      first_name=?, last_name=?, email=?, phone=?, address=?,
      position_id=?, branch_id=?, hire_date=?, base_salary=?, status=?,
      bank_name=?, bank_account=?, bank_branch=?,
      nid=?, dob=?, gender=?, blood_group=?, emergency_contact=?
      ...
      [
        ...
        position_id || null, branch_id || null, hire_date || null, base_salary || 0, status || 'active',
        bank_name || null, bank_account || null, bank_branch || null,
        nid || null, dob || null, gender || null, blood_group || null, emergency_contact || null,
        id,
      ])
    return { ok: true }
  }
  ...
  if (action === 'save_salary_structure') {
    ...
    await query(`INSERT INTO hr_salary_structures
      ...
      [id, basic_salary, house_allowance, transport_allowance, medical_allowance,
       other_allowances, provident_fund, tax_deduction, other_deductions, gross, net])
    return { ok: true }
  }
  ...
  if (action === 'add_attendance') {
    ...
    await query(`INSERT INTO hr_attendance (employee_id, branch_id, date, status, clock_in, clock_out, manual_entry)
      VALUES (?, ?, ?, ?, ?, ?, 1)
      ON DUPLICATE KEY UPDATE
        status=VALUES(status), clock_in=VALUES(clock_in),
        clock_out=VALUES(clock_out), manual_entry=1`,
      [id, body.branch_id || 1, date, status,
  ...
  if (action === 'add_payroll') {
    ...
    await query(`INSERT INTO hr_payrolls
        (employee_id, branch_id, pay_period_start, pay_period_end, gross_salary, deductions, net_salary, status)
      VALUES (?,?,?,?,?,?,?,?)`,
      [id, body.branch_id || 1, pay_period_start, pay_period_end,
       gross_salary, deductions, net_salary, status || 'paid'])
    return { ok: true }
  }
  ...
  if (action === 'add_loan') {
    ...
    await query(`INSERT INTO hr_loans
        (employee_id, branch_id, loan_date, amount, installments, monthly_payment, installment_type, status)
      VALUES (?,?,?,?,?,?,?,?)`,
      [id, body.branch_id || 1, loan_date, amount, installments, monthly,
       installment_type || 'monthly', 'active'])
    return { ok: true }
  }
  ...
  if (action === 'add_leave') {
    ...
    await query(`INSERT INTO hr_leaves (employee_id, branch_id, leave_type, start_date, end_date, reason, status)
      VALUES (?,?,?,?,?,?,?)`,
  ...
  if (action === 'add_document') {
    ...
    await query(`INSERT INTO hr_documents (employee_id, name, category, file_type, file_path, expiry_date, notes)
      VALUES (?,?,?,?,?,?,?)`,
      [id, name, category || 'general', file_type || null,
       file_path || null, expiry_date || null, notes || null])
    return { ok: true }
  }
  ...
  if (action === 'assign_asset') {
    ...
    await query(`INSERT INTO hr_asset_assignments (asset_id, employee_id, assigned_on, due_date, condition_in, notes)
      VALUES (?,?,?,?,?,?)`,
  ...
  if (action === 'return_asset') {
    ...
    await query(`UPDATE hr_asset_assignments SET returned_on=?, condition_out=? WHERE id=?`,
